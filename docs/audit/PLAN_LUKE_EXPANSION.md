...
**Status:** PLANNED — self-audited + **externally AUDITED ✅ APPROVE** (`AUDIT_LUKE_EXPANSION_PLAN.md`, 2026-06-22; findings verified against source + folded in) · approved: AI-draft→provisional→Rule-28 (Q1), EN checkpoint before propagation (Q2), mirror-Matthew companion depth (Q3) · **Date:** 2026-06-22 · **Class:** content authoring (new book, from source) + new-book activation · **Risk:** Medium-High (*authoring from Greek*, not Mark-style propagation; Rule-28 provisional)

> **External-audit disposition (both substantive findings verified against source + folded in):**
> - **Finding 1 — §0.12 cross-book validator is the real "see-target allow-list," and it's TWO hardcoded places + a glob (not just RULES-CORE.md).** Verified: `content-lint.sh` §0.12 scans only `content/*/{genesis,john,matthew,mark}/PEOPLE.md` (line 246 — **Luke absent**) and its `%allowed` hash (line 238) = `genesis/matthew/mark/john (+acts/exodus/kings/isaiah forward-tracked)` — **no `luke`**. So Luke's outgoing genealogy pointers (~76 names → genesis/matthew, the corpus's heaviest cross-book payload) would be **silently unvalidated**. Touchpoint 7 rewritten: (a) RULES-CORE doc allow-list, (b) §0.12 `%allowed` hash `luke => 1`, (c) **§0.12 glob `content/*/luke/PEOPLE.md`** — matching RULES-CORE's own "5 SYNCHRONIZED CHANGES" checklist (step 5).
> - **Finding 2 — §0.11 (DE redundant-parens) glob misses Luke (and already misses Mark).** Verified line 217 = `content/de/{genesis,john,matthew}/CHAPTER-*.md`. DE Luke (genealogy + canticles = dense HB names) is exactly where a `Name (Name)` slip is likely. Touchpoint 6 adds `content/de/luke/CHAPTER-*.md`; **also backfill `content/de/mark/CHAPTER-*.md`** to close the pre-existing Mark gap while there (in scope — it's the same one-line fix and the same regression class).
> - **Minor 1** — corpus default: Luke→Greek Scriptures via `HEBREW_BIBLE = {genesis}` fall-through in *both* `books/page.tsx` + `[book]/page.tsx` (no edit needed); added an explicit gate line. *Recorded for the deferred HB-books plan: Psalms/Prov/Eccl WILL need both `HEBREW_BIBLE` sets edited — a HB-only 9th touchpoint.*
> - **Minor 2** — promote the canticle line-break check to an explicit **Phase-1.0 spike**: author the Magnificat (1:46-55) first, confirm `renderMarkdownSafe` preserves intra-verse line breaks **before** authoring 170 verses (if it collapses them, that's a renderer change to know up front).
> - **Minor 3** — `luke.md` cites the Matthew-*malakh* / Mark precedent for the Gabriel (*angelos*) Rule-30 exclusion, so the Hellinist reviewer sees it's principled.
> - *Note (not Luke's concern): RULES-CORE has a stale `v3.3` schema sample — the same methodology-stamp lag already logged in PENDING.*
...
6. `scripts/content-lint.sh` — register luke in `CONTENT_DIRS`/`STUDY_DIRS`/`PEOPLE_FILES`/`NON_EN_PEOPLE_FILES`/`CONTEXT_FILES` (+ `ES_NT_DIRS`/`ES_NT_CHAPTER_FILES` since Luke is GS); add `docs/editorial-log/luke.md` to `EDITORIAL_LOGS`; **add `content/de/luke/CHAPTER-*.md` to the §0.11 DE-redundant-parens glob (line ~217), and backfill the pre-existing `content/de/mark/CHAPTER-*.md` gap there too** (audit Finding 2).
7. **Cross-book see-target allow-list — THREE synchronized places** (audit Finding 1; RULES-CORE "5 SYNCHRONIZED CHANGES" step 5): (a) `docs/rules/RULES-CORE.md` doc allow-list, (b) `content-lint.sh` §0.12 `%allowed` perl hash (`luke => 1`, line ~238), and (c) the §0.12 **scan glob** (add `content/*/luke/PEOPLE.md`, line ~246) — so Luke's genealogy pointers are actually validated, not exempted.
...
0. **Canticle render spike (FIRST — audit Minor 2):** author just the Magnificat (1:46-55) with intra-verse line breaks; run it through `renderMarkdownSafe` / a built page and confirm the poetic line breaks survive. If they collapse, decide the renderer tweak **before** authoring 170 verses. Cheap; de-risks the one novel rendering question.
...
- **Corpus default (audit Minor 1):** Luke shows corpus = "Greek Scriptures" on /books + book hub (verifies the `HEBREW_BIBLE` fall-through; no edit was needed).
- **§0.12 cross-book lint actually covers Luke:** running `content:lint` validates Luke's outgoing PEOPLE pointers (glob now includes `content/*/luke/PEOPLE.md`) — a mistyped genealogy target warns, not silently dangles.
