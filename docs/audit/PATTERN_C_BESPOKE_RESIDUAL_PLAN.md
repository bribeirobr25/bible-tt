...
**Status:** ⏸️ **EXECUTION PAUSED 2026-06-09 pending two scope decisions (see §2 audit block).** Treatment decisions P-C2-Q1…Q4 remain locked at the recommended defaults (Q1 mirror-EN · Q2 both-senses · Q3 meaning/drop-translit · Q4 overview-only, names follow v3.2 policy), but a 2026-06-09 self-audit found the *scope* was mis-counted and incomplete: counts corrected to **30/30/30 = 90** (symmetric; the earlier asymmetry was a diacritic-scan artifact), plus a newly-found **E-class** (~24 missed bespoke terms) and **D-class** (~17 DE main-pass leftovers). Do not execute until those two are decided. Independently **audited 2026-06-08 → APPROVE** (`AUDIT_PATTERN_C_BESPOKE_RESIDUAL_PLAN.md`); its one substantive note (cite Rule 16 cross-supplement for the *pneuma/ruach* log entry) is folded into §3.3 + §7. A skeptical re-verification (2026-06-08) against RULES-CORE also **corrected a citation error the audit missed**: "Rule 17" was wrongly used for name-rendering/parallel-passage (Rule 17 is *Definite Article Consistency*) — name handling now cites the **v3.2 Name Rendering Policy** and cross-locale consistency cites **Rule 16 / Rule 1**.
...
| **Names** (name policy, not Pattern C) | `havah` ×3, `iesous` ×3 (`yeshua/yehoshua` ×3 = KEEP, protected) | ⏭️ separate name-cleanup |
