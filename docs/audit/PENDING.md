...
- **Glossary fields bypass the markdown renderer (literal `*`).** `src/ui/study/glossary-panel.tsx:16,22` renders `entry.translation` and `entry.notes` as raw `{text}`, so emphasis markers show literally (e.g. "waters *(default)* / water *(contextual)*", "Distinct from *eretz*", "Paired with *demut*"). Pre-existing (the panel never used `renderInlineSafe`); surfaced 2026-06-18 during the drift-fix visual audit. Small scope (~1 cell per chapter, where a glossary field carries `*…*`). Fix: render `translation` via `renderInlineSafe` (straightforward); `notes` needs care — it is *also* the `title=` tooltip attribute and is char-sliced to 60, so HTML can't be naively injected (strip markers for the title + render-or-strip for the truncated display). Low severity / cosmetic.
