...
**Pivot-fidelity audit — DONE 2026-06-23** (`docs/audit/PIVOT_FIDELITY_AUDIT.md`): tested whether mirror-from-EN locales (PT/DE/ES) lose fidelity vs. direct-from-source. Verdict: concern valid in principle, modest in magnitude; one clean pivot artifact confirmed (DE `der Mächtige *Eine*` — calqued EN grammatical filler). Mechanical fixes applied + folded into the Luke-propagation PR; lint guards added (§0.3b ES regressions, §0.3c anti-calque). Two follow-ups remain:

- **Rule-28 / source-anchored review items** (do not self-resolve; for the Hellenist/locale editors, comparing target ↔ **original**, never ↔ English): (a) PT Gen 1:2 `pairando` vs the rulebook's own worked-example `pairava`; (b) PT/ES `ταπείνωσις` (Lk 1:48) rendered as the virtue "humildade/humildad" vs the condition "lowliness/humiliation" (EN "lowliness"/DE "Niedrigkeit" are correct); (c) EN-level: whether John 1:5 `κατέλαβεν` should be a main-text `{a:}` slash rather than the current resolution-plus-Tier-2-note (decide at EN, then propagate). **Process change recorded:** locale Rule-28 sign-off must be source-anchored, and propagation should include an anti-calque reverse-check.

- **Systemic ES diacritic sweep (NEW, pre-existing, larger than the audit sample) — TODO.** The audit surfaced that ES content (esp. the older `es/genesis`, `es/john`) has pervasive diacritic loss beyond the high-precision tokens already fixed: pronoun `él` written `el` mid-phrase (mixed with the legitimate article — needs per-instance judgment: `a el`→`al` before a noun vs `a él` pronoun), plus `¿Donde`→`¿Dónde`, `clamandó`→`clamando`, etc. Not safe to mass-edit (article/pronoun ambiguity) and would false-positive in lint, so deferred to a dedicated tooling-assisted pass (parse-aware or reviewed diff). `content:lint §0.3` only covered ES *NT* dirs and a fixed word-list — it never scanned `es/genesis` and missed these classes; §0.3b now guards the cleared subset, but the full sweep is its own task.
