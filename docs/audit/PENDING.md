...
- **NT Epistles — next: Galatians** (then Romans, James), continuing the recorded post-Acts trajectory; 1 Peter (the genre pilot) is done. *(see the 1 Peter DONE entry below for the trajectory + T-16 genre pattern)*
- **Genesis 13–50 (Phase 12)** — the biggest remaining body of work. *(detail §1)*

**Human review gate:**
- **Rule-28 cross-alignment sign-off** — all 2026-06/07 content ships `provisional` (incl. the Pattern C overview de-jargon, the **§I Option C per-category labels**, and the new **GS books — Luke, Acts, 1 Peter** — awaiting **Hellenist** review); a credentialed source-language scholar / locale-editor must sign off to flip status. AI QA is done; the §I per-category confidence labels + the GS translation cruxes are the specific items the source scholar reviews. *(see EXECUTION_HISTORY 2026-06-10 + 2026-06-12 + the Luke/Acts/1-Peter entries)*
...
**Methodology-stamp drift — DONE 2026-06-21** (`PLAN_METHODOLOGY_RESTAMP.md`, self-audited; commit `b638990` on `methodology-restamp`): re-stamped **72 chapters** `v3.3/29-Rule` → `v3.4/30-Rule` (genesis 1-12, john 1-3, matthew 1-3 × 4 locales; en/mark was already v3.4). It was broader than first thought — the *whole* corpus, not just GS. Verify-then-stamp: the v3.4 markers were already applied (verified balanced + cross-locale-identical; legend 75/75; Rule-30 scope correct). Stamp = ruleset version (orthogonal to `provisional`); per-span semantic sign-off stays with **Rule-28**. Gates green; metadata not rendered.

**Mark 1–3 propagation to PT-BR/DE/ES — DONE 2026-06-21** (`PLAN_MARK_PROPAGATION.md`, self-audited + externally audited APPROVE; branch `mark-propagation`): 27 new files (9 × 3 locales); Mark now four-locale. Mirror-EN, GS-governed (Almeida/Luther/RV familiar names; DE→JHWH, PT/ES→YHWH verified in body cross-refs; marker parity; headers/dual-labels from each locale's Matthew). Gates green (882 tests; conservation 240 files→13112 units; content:lint incl. ES §0.3/§0.4; visual per locale). Ships `provisional` pending **Rule-28** locale-editor sign-off. EXECUTION_HISTORY 2026-06-21; mark.md 005/006/007.

**Luke 1–3 — EN authored + PT-BR/DE/ES propagated — DONE 2026-06-23** (`PLAN_LUKE_EXPANSION.md`, audited APPROVE): book #5 activated + full EN Luke from NA28 (12 files, 2026-06-22), then propagated to the other three locales (36 files; `5810bb3`/`245308a`/`343a810` on `luke-propagation`). Luke now four-locale — **all five books at full locale parity.** Mirror-EN, GS-governed (Almeida/Luther/RV; DE→JHWH, PT/ES→YHWH; marker parity; ES converted to ustedes register per convention). Gates green (886 tests; conservation 288 files/15945 units; content:lint incl. ES §0.3/§0.4/[legacy]-vosotros; visual per locale). Ships `provisional`. EXECUTION_HISTORY 2026-06-23; luke.md L-003/004/005. **Still open:** (b) **Hellenist Rule-28 review** of the translation — applies to all four locales now (cruxes flagged in-text: 1:28 κεχαριτωμένη, 1:35, 1:78, 2:14, 2:49, 3:22 + Ps 2:7 variant, the genealogy divergence from Matthew). Psalms/Proverbs/Ecclesiastes remain deferred (Hebrew-poetry genre + wisdom-file-set question).

**Acts 1–3 — new book #6, EN authored + propagated to all locales — DONE 2026-06-24** (`PLAN_ACTS_EXPANSION.md`, audited APPROVE; branch `acts-expansion`): the first new book under `AUTHORING-PLAYBOOK.md`. Book #6 activated + full EN Acts from NA28 (12 files) + propagation to PT-BR/DE/ES (36 files). **All six books now at full four-locale parity.** GS narrative (Luke vol. 2); Almeida/Luther/RV; DE→JHWH, PT/ES→YHWH; marker parity exact (36/36); ES ustedes. Gates green (886 tests; conservation additive; activation-consistency 8/8; content:lint; build; served-HTML per locale). Ships `provisional`. EXECUTION_HISTORY 2026-06-24; acts.md A-006/007. **Still open:** **Hellenist Rule-28 review** of Acts (all four locales) — cruxes flagged in-text: 2:34 Ps 110 double-κύριος, 2:38 `eis aphesin`, 1:18-19 Judas-death (un-harmonized), 3:15 `archēgos`, 3:13/26 `pais`, 3:21 `apokatastasis`. **Next book (recorded trajectory):** the NT Epistles — **1 Peter** (genre pilot: tightest continuity from Acts 1–3), then Galatians, Romans, James; each its own audited `PLAN_{BOOK}_EXPANSION.md`. The first epistle is a deliberate genre milestone (adapted intro sections, thinner PEOPLE, OT-in-argument — see playbook T-16).

**1 Peter 1–5 — new book #7, the EPISTLE GENRE PILOT, EN authored + propagated to all locales — DONE 2026-07-01** (`PLAN_1PETER_EXPANSION.md` + `AUDIT_1PETER_EXPANSION_PLAN.md`, audited APPROVE; branch `1-peter-expansion`): the pipeline's first *Letter* (playbook T-16). Book #7 activated + full EN 1 Peter from NA28 (18 files — the whole book, first *complete* NT book) + propagation to PT-BR/DE/ES (54 files). **All seven books now at full four-locale parity.** Genre-adapted: §D3 Epistolary Conventions, thin PEOPLE (Kefa see-stub → mark; no genealogy/timeline), OT-in-argument PROPHECY. **Phase 0a:** widened the 2 cross-book slug-parsing regexes for the first leading-digit slug (`1-peter`) + regression test — the durable N-book fix. Mirror-EN, GS-governed (Almeida/Luther/RV; DE→JHWH, PT/ES→YHWH; marker parity exact 54/54 scripted; ES ustedes; PT vós Almeida register; anti-calque). Gates green (887 tests; conservation additive 354→408 files/20,599 units; content:lint; build; visual per locale). Two-pass audit (incl. an independent deeper structural/faithfulness pass) — the two apparent drifts both matched the established Acts convention. Ships `provisional`. EXECUTION_HISTORY 2026-06-26/07-01; `1-peter.md` P-001…P-010. **Still open:** **Hellenist + locale-editor Rule-28 review** (all four locales) — the reviewer flashpoints: 2:18–3:7 household code (P-009), 3:18–20 spirits-in-prison (P-006), 3:21 baptism antitype (P-007), 1:1–2 salutation (P-008), 4:6 (P-010), 2:8 "appointed". **Next book (recorded trajectory):** **Galatians** (tightest Acts interlock), then **Romans**, **James** — each its own audited `PLAN_{BOOK}_EXPANSION.md`.

**Minor — pre-existing unregistered claim label:** `content/en/genesis/CHAPTER-3.md` uses `INTERPRETIVE` (not in `labels.ts` → falls back to TEXTUAL with a console.warn). Found during the Luke audit; pre-dates it. Low-priority sweep item (pick a registered type, e.g. STRONG INFERENCE).

**Cross-book see-stub link bug — DONE 2026-06-22** (`PLAN_CROSSBOOK_SEELINK_FIX.md`, externally audited APPROVE; commit `2624bc8` on `crossbook-seelink-fix`): `finalizeEntry` dropped `crossBookSeeBook` from its return → all cross-book see-stubs rendered plain text instead of links. Fixed with the 1-line pass-through + a 4-case regression test (closing the test gap). Authored targets (genesis/matthew) now render localized-label links across all 4 locales; unauthored forward-refs (exodus/isaiah/kings/acts) keep the graceful plain-text fallback. README "clickable cross-book link" claim now accurate. Gates green (886 tests; conservation 13112 unchanged). Visual confirmed (Docker MCP): authored link renders, dangling stays plain text.
