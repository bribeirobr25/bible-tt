...
- **DRY / reusability consolidation** — `docs/audit/ARCHITECTURE_DRY_AUDIT.md` (2026-06-18). DDD layering is intact; the issue is copy-paste-then-drift around the dual-label concept. **Done:** the 2 active drift-bugs (renderer prose-bold gap; SPECULATIVE color drift); the glossary literal-`*` fix; and **Tier 1 — renderer nested-emphasis hardening** (`PLAN_RENDERER_NESTED_EMPHASIS.md`, executed 2026-06-19): `applyEmphasis` helper extracted (4 copies→1), tempered bold regex (italic-in-bold now renders, ~96 lines × 4 locales auto-fixed), table cells routed through the shared inline pipeline, `content:lint §0.14` unbalanced-`**` guard added. **Still open (Tier 2/3):** (a) the 2 *latent* parser drifts (`parseConfidence`/`parseClaimType` — no content triggers them today) fixed *by extraction*, not hand-sync; (b) consolidation — single `domain/content/labels.ts` (parsers), `ui/shared/confidence-tone.ts` (one tone+i18n map), `<Disclosure>` component (~8 call sites), and the 1027-line `people-parser.ts` split. Each ships behind the standard gate.
