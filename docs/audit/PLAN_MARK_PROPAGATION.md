...
**Status:** PLANNED — **self-audited + externally AUDITED ✅ APPROVE** (`AUDIT_MARK_PROPAGATION_PLAN.md`, 2026-06-21; findings verified against source + folded in) · approved: branch off `main`/PR gate (Q1), PT-BR checkpoint (Q2), AI-draft→provisional→Rule-28 (Q3) · **Date:** 2026-06-21 · **Class:** content authoring (translation) · **Risk:** Medium (volume + per-locale rule compliance; additive, not destructive)

> **External-audit disposition (all findings verified true & folded in):**
> - **Finding 1 (substantive) — section-header byte-match.** Parser matches `##` headers against *hardcoded per-locale Sets* (`CONTINUOUS_READING_HEADERS`, `VERSE_STUDY_HEADERS`, `OVERVIEW_SECTIONS`, `SKIPPED_SECTIONS`, glossary/verse regex — verified `markdown-parser.ts:18–42`). A non-matching header **silently parses to 0 units** (no error). The conservation **Phase-4 completeness guard** (`conservation.test.ts:368`, fails on 0 verses/0 paragraphs) is what catches it — its comment cites the real precedent: *ES John's unaccented "VERSICULO" → 0 verses → empty Notes*. → new **§4.11** (mirror non-EN Matthew headers byte-for-byte) + gate rows **G10/G11**.
> - **Finding 1c — label-warning guard** (`conservation.test.ts:413`) fails if any companion/intro claim-type/confidence label is unrecognized → companion labels must use the registered localized forms (mirror Matthew companions). → gate **G11**.
> - **Minor 1 — marker parity must be scripted**, not eyeballed (conservation checks unit text, not marker counts; a dropped `@@…@@` conserves its text but breaks Rule 30). → **G2** made a concrete grep-count diff.
> - **Minor 2 — DE→JHWH / ES→YHWH in BODY cross-refs**, not just the metadata line. Verified: EN/Mark body has 7 `YHWH` (e.g. "Mark 1:3 — KYRIOS / YHWH", "the speaker is YHWH"); blind mirror-EN would leave DE wrong (DE/Matthew uses `JHWH` 13× in body). → per-locale checklist in §4.5/§4.6.
> - **Minor 3 — v3.4 stamp is the *correct* one** (new Mark will be the only non-EN GS at v3.4; the other non-EN GS chapters are stale v3.3 per the kyrios-audit PENDING item — not this plan's job). Noted in §4.8.
> - Audit's deeper pass verified the remedy's exemplar byte-for-byte: pt-br/de Matthew CHAPTER-1 headers all match the parser Sets → "mirror Matthew" rests on proven-good source.
...
4. **Reading-Guide heading localized:** `GUIA DE LEITURA` / `LESEANLEITUNG` / `GUÍA DE LECTURA` (+ the marker-legend lines localized). **See §4.11 — this is one of the parser-matched headers.**
5. **DE specifics:** speech verbs (sprach/sagte) per established DE convention; article additions not in the Greek marked `*…*` per Rule 11 (GS §article notes, e.g. "vom/zum" contractions); **divine name `JHWH` (not YHWH)** — and this applies to **every body cross-ref note**, not just the metadata line (audit Minor 2). Per-file checklist: grep the new DE file for `YHWH`; every occurrence in a body note where the HB Tetragrammaton is meant must read `JHWH` (e.g. EN/Mark's "Mark 1:3 — KYRIOS / YHWH", "the speaker is YHWH" → `JHWH`). Mirror the DE/Matthew body (`malakh JHWH`, `JHWH-Stellen`).
6. **ES specifics:** divine name **`YHWH` (keep — not JHWH; Spanish J=/x/, RULES-CORE L1226)** in body cross-refs too; pan-Hispanic neutral register (no voseo/vosotros); Reina-Valera tradition = **Option B**.
7. **PT-BR specifics:** Almeida tradition = **Option B**; established locked terms (e.g. *unigênito*) per GS table.
8. **Methodology stamp:** new files carry **`30-Rule … (Ruleset v3.4)`** in each locale's wording (current ruleset; matches en/mark). *Context (audit Minor 3):* new non-EN Mark will be the only non-EN GS content at v3.4 — this is **correct** (it reflects the current ruleset); the other non-EN GS chapters are stale `v3.3` per the kyrios-audit PENDING re-stamp item. Don't let a reviewer flag new-Mark-v3.4 as the anomaly.
...
11. **Structural section headers — copy from the locale's Matthew file, never re-translate (audit Finding 1, highest silent-failure risk).** The parser recognizes section types by byte-matching each `##` header against hardcoded per-locale Sets (`markdown-parser.ts:18–42`): `CONTINUOUS READING` (`LEITURA CONTÍNUA`/`FORTLAUFENDE LESUNG`/`LECTURA CONTINUA`), `VERSE-BY-VERSE STUDY` (`ESTUDO VERSO A VERSO`/`VERS-FÜR-VERS-STUDIE`/`ESTUDIO VERSÍCULO POR VERSÍCULO`), `CHAPTER OVERVIEW` (`OVERVIEW_SECTIONS`), `TABLE OF CONTENTS` (`SKIPPED_SECTIONS`), `GLOSSARY` (`^GLOSS…`), and the verse header (`Versículo`/`Vers`). A header that doesn't byte-match → that section **silently parses to 0 units** (no error; an empty Read/Notes door). **Rule: lift every structural header verbatim from the same-locale `matthew/CHAPTER-N.md`** (a proven-correct exemplar — the audit verified pt-br/de Matthew headers all match the Sets), never hand-translate them. The conservation Phase-4 completeness guard (G10) is the backstop. Likewise **companion claim-type/confidence labels** must use the registered localized forms (mirror the Matthew companions) or the label-warning guard (G11) fails.
...
| G2 | marker parity (**scripted**, audit Minor 1) | concrete grep-count diff per file: `@@`/`{t:}`/`{a:}`/`*…*` counts in each new locale file **equal the EN/Mark counterpart's** (per §4.3). Not an eyeball — conservation checks unit *text*, not marker counts, so a dropped `@@…@@` would pass conservation but break Rule 30. |
...
| G10 | **chapter-completeness guard** (audit Finding 1; `conservation.test.ts:368`) | every new `CHAPTER-N.md` parses **≥1 verse, ≥1 paragraph**, an overview, non-empty Base Text + Methodology, no leaked title block. This is the row that catches a mistranslated structural header (silent-empty door). |
| G11 | **label-warning guard** (audit Finding 1c; `conservation.test.ts:413`) | no new companion/intro file carries an unrecognized claim-type or confidence label (parser warns + falls back on unknown → test fails). Companion labels must use the registered localized forms (mirror Matthew companions). |
