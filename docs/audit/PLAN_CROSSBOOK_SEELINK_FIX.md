...
> **Execution outcome:** 1-line pass-through added to `finalizeEntry` + 4-case regression test. Gates green: G1 **886 tests** (882 + 4 new); G2 conservation **240 files/13112 units unchanged**; G3 lint/build clean; G4 authored links render across all 4 locales (mark→matthew 3×, john→matthew 2×, matthew→genesis 5 stubs); G4b link label = localized book name (verified rendered markup: `…href="/en/matthew/people">Matthew</a>`, `…/de/matthew/people">Matthäus</a>`); G5 dangling targets (exodus/isaiah/kings) stay plain text. **G6 visual (Docker MCP): pending — MCP browser was disconnected at execution; substituted with rendered-markup extraction (no new CSS/component, so visual risk minimal). Capture on reconnect before/after merge.** README's "clickable cross-book link" claim is now accurate (no edit needed).
