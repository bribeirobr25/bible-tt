...
The Transparent Translation (TT) — a multilingual Bible translation project with a web application for reading, studying, and exploring contextual enrichment. The translation currently covers Genesis 1–12, John 1–3, Matthew 1–3, Mark 1–3, Luke 1–3, and Acts 1–3 in English, Brazilian Portuguese, German, and Spanish, governed by a 30-rule system that prioritizes source-text fidelity, ambiguity preservation, and theological restraint.

## Verified state

- **Scope:** Genesis 1–12, John 1–3, Matthew 1–3, Mark 1–3, Luke 1–3, Acts 1–3, 1 Peter 1–5 — **all seven books at full four-locale parity** (EN, PT-BR, DE, ES). Each book has chapters + INTRODUCTION + PEOPLE + study companions (CONTEXT, and PROPHECY where warranted). (Acts propagated 2026-06-24; **1 Peter — the corpus's first *complete* NT book and the EPISTLE GENRE PILOT — EN authored from NA28 2026-06-26 + propagated to PT-BR/DE/ES 2026-07-01**; genre-adapted per playbook T-16: §D3 Epistolary Conventions, thin PEOPLE (no genealogy), OT-in-argument PROPHECY, `1-peter` = the first leading-digit slug [Phase 0a widened 2 cross-book slug regexes]. All provisional pending Hellenist + locale-editor — Rule 28.)
...
- **Tests:** 887 passing (`pnpm test`); `pnpm build`, `pnpm lint`, `pnpm content:lint` all clean. (Conservation gate: 408 files / 20,599 units.)
...
- **Next up:** the **NT Epistles** continue (1 Peter done — the genre pilot) — **Galatians** (tightest Acts interlock), then **Romans**, **James**; each its own audited `PLAN_{BOOK}_EXPANSION.md`, genre-adapted per T-16. Also **Genesis 13–50 (Phase 12)** — the largest remaining work; its §I follows the established Option C structure (collapsible dating scenarios with per-category `**[claim — confidence]**` labels). Then Phase 6 (search) and the cross-book canonical PEOPLE source-merge. All 2026-06/07 content ships `provisional` pending **Rule-28 sign-off** (Hellenist for the GS books; the §I per-category confidence labels are the specific source-scholar review item). See `docs/audit/PENDING.md`.
...
- `pnpm test` — all unit tests (13 files: chapter, enrichment, people, prophecy, introduction, book-context parsers + render-markdown-safe + book-context-real + conservation [Phase 2 structured-layer gate] + labels + confidence-tone + activation-consistency + cx)
...
Governed by `docs/rules/RULES-CORE.md` + `RULES-HB.md` + `RULES-GS.md` (30 rules). Must-know principles:
