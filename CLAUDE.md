...
- **Structured layer (UX/Structure Phase 2, 2026-06-04, uncommitted):** derived, additive `StructuredUnit` layer (`domain/content/ids.ts` + `structured.ts` + `content-loader.getStructuredBook()`) with stable IDs; conservation gate (`__tests__/conservation.test.ts`) proves zero loss in the parser→structured derivation across all 204 files (10,057 units) — every parsed unit round-trips into the structured layer (it does not, by itself, prove the parser extracts every raw-markdown line; the chapter-completeness + inventory guards backstop that). Markdown stays source of truth.
...
- **Next up:** Phase 6 (search) — UX/Structure Phases 1–5 essentially complete (Pattern C glossed 26 non-EN terms; a small bespoke wordplay/grammar residual deferred, see PENDING); then Genesis 13–50 (Phase 12, gated by the deferred Genesis §I "Option C" decision — see PENDING); cross-book canonical PEOPLE source-merge. (§I `#### IA-x` sub-entry decision **resolved 2026-06-08** = Option A, don't migrate; README staleness **fixed 2026-06-08**.)
